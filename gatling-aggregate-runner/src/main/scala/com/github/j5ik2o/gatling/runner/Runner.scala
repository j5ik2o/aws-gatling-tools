...
object Runner extends App with Loop {
  ...
  val underlying: JavaEcsAsyncClient = JavaEcsAsyncClient.builder().build()
  val ecsAsyncClient: EcsAsyncClient = EcsAsyncClient(underlying)

  val slackIncomingWebhookUrlOpt: Option[String] =
  ...
  val chatworkHostOpt: Option[String] = config.getAs[String]("gatling.notice.chatwork.host")
  val chatworkRoomIdOpt: Option[String] =
    config.getAs[String]("gatling.notice.chatwork.room-id")
  val chatworkTokenOpt: Option[String] = config.getAs[String]("gatling.notice.chatwork.token")
  logger.info(
    s"chatworkHostOpt = $chatworkHostOpt, chatworkRoomIdOpt = $chatworkRoomIdOpt, chatworkTokenOpt = $chatworkTokenOpt"
  )

  val runTaskEcsClusterName: String = config.as[String]("gatling.ecs-cluster-name")
  val runTaskTaskDefinition: String = config.as[String]("gatling.task-definition")
  val runTaskCount: Int             = config.as[Int]("gatling.count")
  val runTaskSubnets: Seq[String]   = config.as[Seq[String]]("gatling.subnets")
  val runTaskAssignPublicIp: AssignPublicIp =
    AssignPublicIp.valueOf(config.as[String]("gatling.assign-public-ip"))

  val runTaskLogPrefix: String = config.as[String]("gatling.log-prefix")
  val runTaskContainerOverrideName: String =
    config.as[String]("gatling.container-override-name")

  val df                      = new SimpleDateFormat("YYYYMMDDHHmmss")
  val executionIdPath: String = runTaskLogPrefix + df.format(now) + "-" + now.getTime.toString

  logger.info(s"executionIdPath = $executionIdPath")

  val runTaskEnvironments: Map[String, String] = config.as[Map[String, String]](
    "gatling.environments"
  ) ++ Map("GATLING_EXECUTION_ID" -> executionIdPath)

  val runTaskReporterTaskDefinition: String =
    config.as[String]("gatling.reporter.task-definition")
  val runTaskReporterContainerOverrideName: String =
    config.as[String]("gatling.reporter.container-override-name")

  val runTaskReporterEnvironments: Map[String, String] = config.as[Map[String, String]](
    "gatling.reporter.environments"
  ) ++ Map("GATLING_RESULT_DIR_PATH" -> executionIdPath)

  import scala.concurrent.ExecutionContext.Implicits.global

  val futureTasks: Future[Seq[Task]] = if (runTaskCount > 10) {
  ...
  val future: Future[Unit] = futureTasks.flatMap { tasks =>
